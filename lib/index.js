...
/** 「可回传」的迹象：可选中复制的汇总区，或交给卡片。 */
const RECEIPT_SUMMARY_SIGNALS = [/<textarea[^>]*\breadonly/i, /postMessage/]
...
function receiptHint(html) {
  ...
  if (!hasSummary) missing.push('a copyable summary (`<textarea readonly>` and/or `postMessage`)')

  return [
    'ⓘ Host check: this page does not look like it lets the user reply — missing',
    `${missing.join(' + ')}.`,
    'Every showme page is expected to carry the reply block: load the `showme-report` skill (§3)',
...
function buildTool(ctx) {
  return {
    ...
    async execute(args, exec) {
      ...
      // 回执体检：页面缺"让用户回话"的部分时，提醒会随工具结果回到模型眼前。
      let hint = ''
      try {
        hint = receiptHint(await readFile(located.path, 'utf8'))
      } catch {
        /* 读不到就只是不提醒，展示照常 */
      }

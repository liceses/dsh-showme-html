...
function buildTool(ctx) {
  return {
    ...
    description: [
      ...
      'If the page collects verdicts or choices from the user, say so in your reply — the page itself gives the user a text block to send back, and what they send lands in `.dsh/showme/inbox.jsonl`.',
    ...
    output: {
      ...
      render(_args, value) {
        return [{ type: 'text', text: `Already shown to the user in the conversation: ${value?.path}` }]
      },
